        ...
        <div className=" w-full h-screen relative flex flex-col    mt-20" style={{ fontFamily: "Belleza" }}>
            ...
            <div className="flex flex-col absolute top-0 cursor-pointer  justify-evenly    pt-10 p-10  h-full ">
                {titlesImgs.map(tm =>
                    ...
                    <h2 key={tm.title} className="md:text-6xl text-xl text-white font-extrabold  box-content transition ease-in duration-150 hover:text-green-600" onMouseOver={() => setChoosedImg(tm.imgName)} onClick={() => setChoosedImg(tm.imgName)}>  {tm.title} </h2>
